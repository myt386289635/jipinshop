 ...
 * @Describe
 */
public class ShopUserPresenter {

    private Repository mRepository;
    private ShopUserView mView;

    public void setView(ShopUserView view) {
        mView = view;
    }

    @Inject
    public ShopUserPresenter(Repository repository) {
    ...
    public void solveScoll(RecyclerView mRecyclerView, final SwipeToLoadLayout mSwipeToLoad,
        ...
        appBarLayout.addOnOffsetChangedListener((appBarLayout1, verticalOffset) -> {
            if (once[0] || mRecyclerView.getVisibility() == View.GONE) {//数据为空时
                if (verticalOffset == 0){
                    //展开时
    ...
    public void getDate(String id ,int page, LifecycleTransformer<FreeUserListBean> transformer){
        mRepository.freeUserList(id,page)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .compose(transformer)
                .subscribe(freeUserListBean -> {
                    if (freeUserListBean.getCode() == 0){
                        if (mView != null){
                            mView.onSuccess(freeUserListBean);
                        }
                    }else {
                        if (mView != null){
                            mView.onFile(freeUserListBean.getMsg());
                        }
                    }
                }, throwable -> {
                    if (mView != null){
